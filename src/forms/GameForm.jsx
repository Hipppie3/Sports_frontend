...
const GameForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await axios.put(`${import.meta.env.VITE_API_BASE_URL}/games/${editingId}`, formData);
        setEditing(false);
        setEditingId(null);
      } else {
        await axios.post(`${import.meta.env.VITE_API_BASE_URL}/games`, formData);
      }
      setGame({
        game_date: '', game_time: '', home_team_id: '', away_team_id: '',
        location: '', home_team_points: 0, away_team_points: 0, video_url: ''
...
const handleEdit = (game) => {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/games/${id}`);
        fetchGames();
      } catch (error) {
